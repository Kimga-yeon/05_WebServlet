...
$("#btn").click(function(){ // 생성 버튼 클릭 시
   ...
   for(var i=0; i<cellCount*cellCount ; i++){ // cellCount*cellCount 만큼반복
       ...
       // 중복 제거
       var flag = true; // 중복 여부 확인용 변수

       for(var j=0 ; j<ranArr.length ; j++){ // 현재 ranArr 배열의 길이만큼 반복
           if(ranArr[j] == ran){ // 중복되는 값이 있을 경우
               i--; // 바깥쪽 for문의 반복용 값 i를 1감소
               flag = false; // false일 경우 중복이 발생한 것.
               break;
           }
       }

       if(flag){ // 중복이 아닐 경우
           ranArr.push(ran); // 배열에 랜덤 값 추가
       }
   }
